...
int main() {
    std::cin >> a >> b;
    int len1 = a.length(), len2 = b.length(), last = 0;
    a = ' ' + a, b = ' ' + b;
    for (int i = 1; i <= len1; ++i) pre[i] = len2, suf[i] = 1;
    suf[len1 + 1] = len2 + 1;
    for (int i = 1, j = 1; i <= len1 && j <= len2; ++i) {
        pre[i] = last = a[i] == b[j] ? j++ : last;
    }
    last = len2 + 1;
    for (int i = len1, j = len2; i >= 1 && j >= 1; --i) {
        suf[i] = last = a[i] == b[j] ? j-- : last;
    }
    int ans = len2 + 1, left = 1, right = len2 + 1;
    for (int i = 1; i <= len1 + 1; ++i) {
        if (pre[i - 1] < suf[i] && ans > suf[i] - pre[i - 1]) {
            ans = (right = suf[i]) - (left = pre[i - 1]);
        }
    }
    if (ans == len2 + 1) {
        std::cout << "-" << std::endl;
    ...
    for (int i = 1; i <= len2; ++i) {
        if (i <= left || i >= right) std::cout << b[i];
    }
    std::cout << std::endl;
    return 0;
}
