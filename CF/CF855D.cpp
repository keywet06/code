// oct code object CF/CF855D.cpp
#ifndef OCT_CODE_OBJECT_CFx2fCF855Dx2eCPP
#define OCT_CODE_OBJECT_CFx2fCF855Dx2eCPP
...
int i, j, k, n, q;
int in[N], cp[N], de[N], f[N];
int fa[N][C];
std::vector<int> v[N], v2[N];

int fi(int u) { return f[u] == u ? u : f[u] = fi(f[u]); }
void dfs(int u, int p, int h, int c) {
    fa[u][0] = p, in[u] = h, cp[u] = c, de[u] = de[p] + 1;
    for (int i = 1; i <= 20; i++) {
        fa[u][i] = fa[fa[u][i - 1]][i - 1];
    }
    for (int i = 0; i < v[u].size(); i++) {
        if (v[u][i] == p) continue;
        v2[u][i] == 0 ? dfs(v[u][i], u, h + 1, c) : dfs(v[u][i], u, h, c + 1);
    }
}
int lca(int u, int v) {
    if (de[u] > de[v]) std::swap(u, v);
    for (int i = 20; i >= 0; i--) {
        if (de[fa[v][i]] >= de[u]) v = fa[v][i];
    }
    if (u != v) {
        for (int i = 20; i >= 0; i--) {
...
int main() {
    ...
    for (i = 1; i <= n; i++) {
        std::cin >> j >> k;
        if (j == -1 && k == -1) continue;
        v[i].pub(j), v[j].pub(i), v2[i].pub(k), v2[j].pub(k), f[fi(i)] = fi(j);
    }
    for (i = 1; i <= n; i++) {
        if (fa[i][0] == 0) dfs(i, i, 0, 0);
    }
    std::cin >> q;
    while (q--) {
        std::cin >> i >> j >> k;
...
#endif  // oct code end CF/CF855D.cpp