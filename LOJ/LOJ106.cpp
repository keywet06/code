// Octopen: from (#include "octinc/splay.hpp")
// Octopen: open up "octinc/splay.hpp"
...
class Splay {
   ...
   public:
    point *root;
    Splay() { root = nullptr; }
    inline int get(point *v) { return !v || !v->fa ? -1 : v == v->fa->son[1]; }
...
#endif
// Octopen: end (open up "octinc/splay.hpp")
// Octopen: end (from (#include "octinc/splay.hpp"))
const int N = 100005;
int n, m;
int a[N];
Octinc::Splay sp[N];
inline void build(int v, int l, int r) {
    for (int i = l; i <= r; ++i) sp[v].insert(a[i]);
    if (l == r) return;
    int mid = l + r >> 1;
    build(v << 1, l, mid);
    build(v << 1 | 1, mid + 1, r);
