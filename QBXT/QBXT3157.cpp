...
const size_t MillerRobinDefaultTestTimes = 16;

template <typename Type>
inline bool MillerRobin(Type n, std::mt19937_64 &Rand,
                        const size_t TestTimes = MillerRobinDefaultTestTimes) {
...
template <typename Type>
inline bool MillerRobin(Type n,
                        const size_t TestTimes = MillerRobinDefaultTestTimes) {
    std::mt19937_64 Rand(GetRand());
    return MillerRobin(n, Rand, TestTimes);
}
...
inline RetType GetFac(Type n, std::mt19937_64 &Rand,
    ...
    if (MillerRobin(n, Rand, TestTimes)) {
        RetType Ret;
        Ret.push_back(n);
        return Ret;
    }
...
int main() {
    ...
    for (auto x : Ans) std::cout << int64(x) << '\n';
    return 0;
}