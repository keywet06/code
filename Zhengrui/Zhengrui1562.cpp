// oct code object Zhengrui/Zhengrui1563.cpp
#ifndef OCT_CODE_OBJECT_ZHENGRUIx2fZHENGRUI1563x2eCPP
#define OCT_CODE_OBJECT_ZHENGRUIx2fZHENGRUI1563x2eCPP
...
int main() {
    ...
    while (--k) {
        now = q.top(), q.pop();
        std::cout << now.fir << '\n';
        if (now.sec == n) continue;
        q.push({now.fir + a[now.sec + 1], now.sec + 1});
        q.push({now.fir + a[now.sec + 1] - a[now.sec], now.sec + 1});
    }
    return 0;
}
