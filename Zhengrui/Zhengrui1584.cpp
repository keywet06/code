// oct code title Zhengrui/Zhengrui1584.cpp
...
inline void solve(int l, int r) {
    if (l > r) return;
    int bl = l, br = r, len = r - l + 1, flag = 0, mid;
    if (len <= ans) return dec(l, r);
    while (bl <= br) {
        if (t[c[bl]] < f[len] || t[c[br]] < f[len]) {
            flag = 2 - (t[c[bl]] < f[len]), mid = (flag ^ 2 ? bl : br);
            break;
        }
        ++bl, --br;
    }
    if (!flag) return ans = len, dec(l, r);
    if (flag == 1) {
        dec(l, mid), solve(mid + 1, r);
        inc(l, mid - 1), solve(l, mid - 1);
    } else {
        dec(mid, r), solve(l, mid - 1);
        inc(mid + 1, r), solve(mid + 1, r);
    }
}
