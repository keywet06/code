// oct code object Zhengrui/Zhengrui1591.cpp
...
void getit(int *x, const int &y);
void getto(int *x, int *y);

char n[M];
int m, r, tim, lenn, epore;
int dps[N], out[N], duc[N], nts[N];

void getit(int *x, const int &y) {
    for (int i = 0; i < m; i++) duc[i] = x[i];
    for (int i = 0; i < m; i++) {
        x[i] += duc[(i - y + m) % m];
        if (x[i] >= P) x[i] -= P;
    }
...
void getto(int *x, int *y) {
    for (int i = 0; i < m; i++) duc[i] = x[i], nts[i] = y[i], x[i] = 0;
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < m; j++) {
            x[(i + j) % m] = (x[(i + j) % m] + 1ll * duc[i] * nts[j]) % P;
        }
    }
    return;
}
int main() {
    ...
    for (int i = 1; i <= lenn; i++) {
        epore = epore * 10 + n[i] - '0';
        tim = (tim * 10ll + epore / m) % (P - 1);
        epore %= m;
    }
    dps[0] = out[0] = 1;
    for (int i = 0; i < m; i++) getit(dps, i);
    while (tim) {
        if (tim & 1) getto(out, dps);
        getto(dps, dps), tim >>= 1;
    }
    for (int i = 0; i < epore; i++) getit(out, i);
    std::cout << out[r] << std::endl;
    return 0;
}

#endif  // oct code end Zhengrui/Zhengrui1591.cpp
