...
function showStandings() {
	...
	for (var i in standings) {
		for (var j = 0; j < problems.length; ++j) {
			...
			if (resultBetter(col, best[j])) best[j] = col.concat(cur);
		}
	}
	$("#standings").long_table(
		...
		function(row) {
			...
			if (row[4] != undefined) {
				col_tr += '<sup>[';
				col_tr += '<span class="uoj-score" data-max="' + problems.length * 100 + '" style="color: ' + getColOfScore(row[4] / problems.length) + '">' + row[4] + '</span>';
				col_tr += ']</sup>';
			}
			col_tr += '<span class="uoj-score" data-max="' + problems.length * 100 + '" style="color: ' + getColOfScore(row[0] / problems.length) + '">' + row[0] + '</span>';
			if (row[4] != undefined) {
			...
			for (var i = 0; i < problems.length; i++) {
				...
				if (col !== undefined) {
					...
					col_tr += '<a href="/submission/' + col[2] + '" class="uoj-score" style="color: ' + getColOfScore(col[0]) + '">' + col[0] + '</a>';
